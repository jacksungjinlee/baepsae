    ...
    for fs in ("CFS", "OFS"):
        try:
            ...
            for row in j.get("list", []):
                ...
                prv = _num(row.get("frmtrm_amount"))
                bfr = _num(row.get("bfefrmtrm_amount"))
                if nm in ("매출액", "수익(매출액)", "영업수익") and rev[0] is None:
                    rev = [cur, prv, bfr]
                elif nm == "영업이익" and op is None:
                    op = cur
                elif nm in ("당기순이익", "당기순이익(손실)") and ni is None:
                    ni = cur
            ...
            if ni and eq and eq > 0:
                out["roe"] = round(ni / eq * 100, 1)
            if op and rev[0]:
                out["opm"] = round(op / rev[0] * 100, 1)
...
def fetch_korea(start, end, rf):
    ...
    try:
        ...
        log(f"· PER/PBR/배당 {len(fundamentals):,}건")
        try:
    ...
    return enrich_with_dart(out), bench
...
def main():
    ...
    log(f"뱁새 데이터 수집 v10 · {start} ~ {end} · DART {'ON' if DART_KEY else 'OFF'}")

    rf = fetch_rf()
    kr, kr_bench = fetch_korea(start, end, rf)
