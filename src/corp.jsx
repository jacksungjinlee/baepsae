...
const EXPL = {
  ...
  ff: { t: "업종 내 위치", b: "지표 하나로는 회사를 판단할 수 없어요. PER·PBR·PSR·EV/EBITDA 같은 여러 지표에서 같은 업종 회사들 가운데 낮은 쪽인지 높은 쪽인지를 나란히 놓은 그림이에요. 파란 상자는 업종의 25~75% 구간, 주황 점이 이 회사의 위치예요. 낮다고 곧 싸다는 뜻은 아니에요 — 시장이 왜 이 위치에 두었는지 물어보는 출발점이에요." },
  ...
  factor: { t: "성향(팩터)", b: "숫자는 0~100 백분위예요. 가치 78이면 PBR 낮은 순으로 시장 상위 22%라는 뜻이에요. 다섯 성향의 기준: 가치(PBR 낮은 순) · 퀄리티(ROE 높은 순) · 모멘텀(최근 12개월 수익률 순) · 규모(시가총액 작은 순) · 저변동(변동성 낮은 순). 역사적으로 남다른 수익 패턴이 관찰됐지만, 10년 넘게 안 통한 시기도 있었어요. 내가 어느 성향에 몰려 있는지 아는 것이 이 숫자의 쓸모예요." },
...
export const SCREEN_COLS = [
  ...
  { k: "vz", ko: "가치", grp: "팩터", fmt: (v) => v + "" },
  { k: "qz", ko: "퀄리티", grp: "팩터", fmt: (v) => v + "" },
  { k: "mz", ko: "모멘텀", grp: "팩터", fmt: (v) => v + "" },
  { k: "pz", ko: "규모", grp: "팩터", fmt: (v) => v + "" },
  { k: "lz", ko: "저변동", grp: "팩터", fmt: (v) => v + "" },
];
const DEFAULT_COLS = ["cap", "per", "pbr", "roe", "dy", "r3"];
const FACTORS = [["vz", "가치", "PBR 낮은 순"], ["qz", "퀄리티", "ROE 높은 순"], ["mz", "모멘텀", "12개월 수익률 순"], ["pz", "규모", "시가총액 작은 순"], ["lz", "저변동", "변동성 낮은 순"]];
  ...
  return (
    <div>
      <Card>
        ...
        <div style={{ fontSize: 10.5, color: C.faint, marginTop: 12 }}>각 성향의 정렬 기준을 따라 상위만 남겨요. "가치 상위 30%" = PBR 낮은 순으로 30%.</div>
        <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 8 }}>
          {FACTORS.map(([k, ko, formal]) => (
            <div key={k} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ width: 104, flexShrink: 0 }}><span style={{ display: "block", fontSize: 11.5, fontWeight: 800, color: C.ink }}>{ko}</span><span style={{ display: "block", fontSize: 9, color: C.faint, marginTop: 1 }}>{formal}</span></span>
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(28,43,69,0.5)", zIndex: 90, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: "16px 16px 0 0", padding: "20px 18px calc(24px + env(safe-area-inset-bottom))", width: "100%", maxWidth: 640, maxHeight: "84vh", overflowY: "auto", fontFamily: FONT }}>
        ...
        {g.tldr && (
          <div style={{ marginTop: 12, background: C.bg, border: HAIR, borderRadius: 10, padding: "12px 14px" }}>
            {g.tldr.map((x, i) => <div key={i} style={{ fontSize: 12.5, fontWeight: 800, color: C.ink, lineHeight: 1.7 }}>· {x}</div>)}
            {g.tags && <div style={{ display: "flex", gap: 6, marginTop: 8 }}>{g.tags.map((tg) => <span key={tg} style={{ border: "1px solid " + C.line, borderRadius: 999, padding: "3px 10px", fontSize: 10, fontWeight: 700, color: C.sub }}>{tg}</span>)}</div>}
          </div>
        )}
        {g.eq && (
          <div style={{ marginTop: 14 }}>
            <div style={{ fontSize: 12.5, fontWeight: 800, color: C.apricotDeep }}>수익 방정식</div>
            <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 7, marginTop: 8 }}>
              {g.eq.map(([txt, kind], i) => kind === "" 
                ? <span key={i} style={{ fontSize: 15, fontWeight: 800, color: C.faint }}>{txt}</span>
                : <span key={i} style={{ border: "1.6px solid " + (kind === "+" ? C.teal : kind === "-" ? C.coral : C.ink), color: kind === "+" ? C.teal : kind === "-" ? C.coral : C.ink, borderRadius: 9, padding: "6px 11px", fontSize: 12.5, fontWeight: 800 }}>{txt}</span>)}
            </div>
            {g.eqnote && <div style={{ fontSize: 10.5, color: C.faint, marginTop: 7, lineHeight: 1.6 }}>{g.eqnote}</div>}
            {g.lev && <div style={{ fontSize: 11.5, color: C.sub, marginTop: 8, lineHeight: 1.65, background: C.bg, borderRadius: 8, padding: "9px 11px" }}>{g.lev}</div>}
          </div>
        )}
        ...
        {g.switches && (
          <div style={{ marginTop: 14 }}>
            <div style={{ fontSize: 12.5, fontWeight: 800, color: C.apricotDeep }}>이 업종을 움직이는 스위치</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 8 }}>
              {g.switches.map(([a, b, dir], i) => (
                <div key={i} style={{ display: "flex", alignItems: "baseline", gap: 8, border: HAIR, borderRadius: 9, padding: "9px 12px" }}>
                  <span style={{ fontSize: 12, fontWeight: 800, color: C.ink, flexShrink: 0 }}>{a}</span>
                  <span style={{ fontSize: 12, fontWeight: 800, color: dir === "+" ? C.teal : C.coral }}>→ {b}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 4, flexWrap: "wrap" }}>
              {g.phases.map((p, i) => (
                <React.Fragment key={p}>
                  <span style={{ border: "1.4px solid " + C.line, borderRadius: 8, padding: "5px 9px", fontSize: 10.5, fontWeight: 800, color: C.ink }}>{p}</span>
                  {i < g.phases.length - 1 && <span style={{ color: C.faint, fontSize: 11 }}>→</span>}
                </React.Fragment>
              ))}
              <span style={{ color: C.faint, fontSize: 11 }}>↻</span>
            </div>
            {g.marker && <div style={{ fontSize: 11, fontWeight: 800, color: C.apricotDeep, marginTop: 7 }}>{g.marker}</div>}
          </div>
        )}
        <Sec title="봐야 할 지표" items={g.metrics} />
        <Sec title="흔한 함정" items={g.traps} />
        {g.ask && (
          <div style={{ marginTop: 16, border: "1.6px solid " + C.ink, borderRadius: 11, padding: "13px 15px" }}>
            <div style={{ fontSize: 12.5, fontWeight: 800, color: C.ink }}>사기 전에 물을 세 가지</div>
            {g.ask.map((q, i) => (
              <div key={i} style={{ display: "flex", gap: 9, marginTop: 8, alignItems: "flex-start" }}>
                <span style={{ width: 18, height: 18, borderRadius: 999, background: C.ink, color: "#fff", fontSize: 10.5, fontWeight: 800, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{i + 1}</span>
                <span style={{ fontSize: 12, color: C.ink, lineHeight: 1.6 }}>{q}</span>
              </div>
            ))}
          </div>
        )}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <Card>
        ...
        {(() => {
          ...
          return <Verdict>{`최근 3개월, 돈은 ${nm(secs[0].k)}·${nm(secs[1].k)}로 몰렸고 ${nm(secs[secs.length - 1].k)}에서 빠졌어요.`}</Verdict>;
        })()}
        <div style={{ marginTop: 10 }}>
...
function DecompCard({ c, setExplain }) {
  ...
  if (d) {
    const epsThen = c.ni[2] * 100 / c.shm;
    const priceThen = c.price / d.priceRatio;
    const perThen = priceThen / epsThen;
    const perNow = c.price / c.eps;
    const won = (v) => Math.round(v).toLocaleString() + "원";
    const up = d.priceRatio >= 1;
    const eDom = Math.abs(Math.log(d.eF)) >= Math.abs(Math.log(d.mF));
    verdict = up
      ? (eDom ? "지난 3년 상승은 대부분 이익이 만들었어요. 이런 상승이 오래가요."
              : "지난 3년 상승은 이익보다 기대가 만든 몫이 커요. 기대는 식으면 되돌아가요.")
      : (eDom ? "주가가 내린 만큼 이익도 줄었어요. 가격보다 실적이 문제였어요."
              : "이익보다 기대가 더 크게 꺾였어요. 시장이 이 회사를 다시 보고 있어요.");
    body = (
      <>
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr auto 1fr", gap: 6, alignItems: "center", marginTop: 12, textAlign: "center" }}>
          <div style={{ background: C.bg, border: HAIR, borderRadius: 10, padding: "13px 6px" }}>
            ...
            <div style={{ fontSize: 9.5, color: C.faint }}>{won(epsThen)} → {won(c.eps)}</div>
          </div>
          ...
          <div style={{ background: C.bg, border: HAIR, borderRadius: 10, padding: "13px 6px" }}>
            ...
            <div style={{ fontSize: 9.5, color: C.faint }}>{perThen.toFixed(1)}배 → {perNow.toFixed(1)}배</div>
          </div>
          <div style={{ fontSize: 16, fontWeight: 800, color: C.faint }}>=</div>
          <div style={{ background: C.ink, borderRadius: 10, padding: "13px 6px" }}>
            ...
            <div style={{ fontSize: 9.5, color: "rgba(255,255,255,0.55)" }}>{won(priceThen)} → {won(c.price)}</div>
          </div>
        </div>
        <div style={{ fontSize: 10.5, color: C.faint, marginTop: 10, lineHeight: 1.6 }}>
          지난 3년을 둘로 나눠본 것뿐, 예측이 아니에요. 주식수 변화와 배당은 눈높이 쪽에 함께 들어 있어요.
        </div>
      </>
    );
  } else {
    body = (
      <div style={{ fontSize: 12, color: C.sub, marginTop: 12, lineHeight: 1.7 }}>
        적자 구간이 있어서 이익과 눈높이로 나누기 어려워요.
        {c.r36 != null && <> 지난 3년 주가는 {c.r36 >= 0 ? "+" : ""}{c.r36}% {c.r36 >= 0 ? "올랐어요" : "내렸어요"}. 이익 없이 움직인 가격은 기대와 수급이 만든 거예요.</>}
      </div>
    );
  }
  return (
    <Card>
      <H num="04" main="주가는 무엇으로 움직였나" sub="지난 3년 주가 = 이익 변화 × 눈높이 변화" onWhy={() => setExplain("decomp")} />
        ...
        <H num="01" main="투자지표" sub={"밸류에이션 멀티플 · 업종 사분위 대비" + (c.bq ? " · 최근 4개분기 합산(" + c.bq + ")" : " · 직전 사업보고서")} onWhy={() => setExplain("box")} />
          {[c.vz, c.qz, c.mz, c.pz, c.lz].some((v) => v != null) && (
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10, padding: "9px 11px", background: C.bg, border: HAIR, borderRadius: 8, alignItems: "center" }}>
              ...
              {[["vz", "가치"], ["qz", "퀄리티"], ["mz", "모멘텀"], ["pz", "규모"], ["lz", "저변동"]].map(([k, ko]) => c[k] == null ? null : (
      ...
      <div className="cgrid2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Card>
          ...
          {(() => {
            ...
            let v;
            if (rUp && nUp === true) v = "매출과 이익이 같이 늘고 있어요. 좋은 흐름이에요.";
            else if (rUp && nUp === false) v = "매출은 느는데 이익이 줄어요. 남는 장사인지 봐야 해요.";
            else if (!rUp && nUp === true) v = "매출은 줄었지만 이익은 늘었어요. 몸집 대신 수익성을 챙겼어요.";
            else if (!rUp && nUp === false) v = "매출도 이익도 줄고 있어요. 업황 탓인지 회사 탓인지가 관건이에요.";
            else return null;
        ...
        <Card>
          ...
          {(() => {
            ...
            const v = hi > lo
              ? "여러 지표에서 업종 중간보다 비싸요. 그만한 이유가 있는지 볼 차례예요."
              : hi < lo
              ? "여러 지표에서 업종 중간보다 싸요. 싼 데는 이유가 있는지 먼저 보세요."
              : "지표마다 평가가 달라요. 이 업종에 맞는 지표부터 고르세요.";
            return <Verdict>{v}</Verdict>;
          })()}
          {ffRows.length === 0 ? <Sub style={{ marginTop: 8 }}>표시할 지표가 없어요. 데이터 갱신 후 채워져요.</Sub> : (
    ...
    <Card>
      ...
      <div className="cgrid2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {N != null && N <= 0 && <Sub style={{ color: C.coral, fontSize: 11 }}>순이익이 0 이하라 이 단순 모형으로는 계산할 수 없어요. 흑자 전환 가정을 직접 입력해보거나, 이 회사엔 다른 지표가 필요해요.</Sub>}
        </div>
      </div>
